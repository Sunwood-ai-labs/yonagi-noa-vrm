...
AI人狼向けの10モーションを、GAME / IDLE / TALKの3カテゴリから選択・再生できます。現在収録しているVRMAは、JSONへ記述した回転キーフレームをText-To-VRMA v1.1.4の`spec2vrma.mjs`で変換したプロトタイプです。ARDYや学習済みモーション生成モデルの出力ではなく、特に待機・トーク6本は下半身ボーントラックを持たないため、AI生成の全身モーションとしては扱っていません。
...
この監査は手作業キーフレームの静止画確認であり、自然なAI生成全身モーションの
証明ではありません。
